...
const ShareButton = ({ 
  ...
  customText = null,
  customUrl = null,
  platforms = ['facebook', 'twitter', 'whatsapp'] // New prop tocontrol which platforms to show 
}) => {
  ...
  const getShareData = () => {
    ...
    const defaultText = `Help support: ${campaign.name}! ${formatNumber(campaign.current_amount)} MRU raised of ${formatNumber(campaign.target)} MRU so far. Join ${formatNumber(campaign.number_of_donors)} donors in making a difference!`;
    
    return {
      title: `Help Support: ${campaign.name}`,
      description: `${formatNumber(campaign.current_amount)} MRU raised of ${formatNumber(campaign.target)} MRU goal • ${formatNumber(campaign.number_of_donors)} donors • ${campaign.description?.substring(0, 100)}...`,
      text: customText || defaultText,
      url: baseUrl,
      image: campaign.files?.[0]?.url,
      hashtags: ['fundraising', 'support', 'donate'].join(',')
    };
  };
  ...
  if (variant === 'button-only') {
    return (
      <button
        ...
        className={`flex items-center space-x-2 text-slate-600 hover:text-blue-600 transition-colors ${className}`}
      ...
      <div className={`bg-white rounded-lg border border-slate-200 p-4 ${className}`}>
        ...
        {facebookNotification && (
          <div className="mb-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-700">
            📋 Campaign text copied to clipboard! Paste it in your Facebook post.
          </div>
        )}
  ...
  return (
    <div className={`bg-white rounded-xl border border-slate-200 p-6 ${className}`}>
      <h3 className="text-lg font-semibold text-slate-900 mb-4">Share This Campaign</h3>
      ...
      <div className="space-y-4">
        ...
        {facebookNotification && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-sm text-blue-800">
              📋 <strong>Campaign text copied to clipboard!</strong> Paste it in your Facebook post for better engagement.
            </p>
          </div>
        )}
        ...
        {showPreview && (
          <div className="border border-slate-200 rounded-lg p-4 bg-slate-50">
            <p className="text-xs text-slate-600 mb-3">Social Media Preview:</p>
              ...
              <div className="flex space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-600 text-xs mt-1">
                    {formatNumber(campaign.current_amount)} MRU raised of {formatNumber(campaign.target)} MRU goal • {formatNumber(campaign.number_of_donors)} donors
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-3">
          <p className="text-sm font-medium text-slate-700">Share directly on:</p>
            ...
            {platforms.includes('facebook') && (
              ...
              >
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10">
                  Opens Facebook + copies text to clipboard
        ...
        <div className="border-t border-slate-200 pt-4">
          <p className="text-sm font-medium text-slate-700 mb-2">Or copy link:</p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-xs text-blue-800">
            💡 <strong>Pro tip:</strong> When you share on social media, your campaign image and progress will be displayed automatically to attract more supporters!
          </p>
        </div>
      </div>
    </div>
