...
export const exportAnalyticsExcel = async (params = {}) => {
  try {
    ...
    console.log('AnalyticsAPI: Exporting Excel with params:', queryParams);

    // Use your custom axios instance but handle the response differently for file downloads
    const response = await api.get('/org/analytics/export_excel/', { 
      params: queryParams,
      responseType: 'blob' // Important for file downloads
    });

    console.log('AnalyticsAPI: Excel export successful');
    
    // Create a response-like object that matches the downloadFile function expectations
    return {
      blob: () => Promise.resolve(response),
      headers: {
        get: (headerName) => {
          // Try to get the filename from the response if available
          if (headerName === 'Content-Disposition') {
            return `attachment; filename="analytics_export.xlsx"`;
          }
          return null;
        }
      }
    ...
    console.error('AnalyticsAPI: Failed to export Excel:', error);
    throw {
...
export const exportAnalyticsPDF = async (params = {}) => {
  try {
    ...
    console.log('AnalyticsAPI: Exporting PDF with params:', queryParams);

    // Use your custom axios instance but handle the response differently for file downloads
    const response = await api.get('/org/analytics/export_pdf/', { 
      params: queryParams,
      responseType: 'blob' // Important for file downloads
    });

    console.log('AnalyticsAPI: PDF export successful');
    ...
    return {
      ...
      headers: {
        get: (headerName) => {
          // Try to get the filename from the response if available
          if (headerName === 'Content-Disposition') {
            return `attachment; filename="analytics_export.pdf"`;
          }
          return null;
        }
      }
    ...
    console.error('AnalyticsAPI: Failed to export PDF:', error);
    throw {
...
export const downloadFile = async (response, defaultFilename) => {
  try {
    ...
    // Check if it's our custom response object from axios
    if (response.blob && typeof response.blob === 'function') {
      blob = await response.blob();
    } else if (response instanceof Blob) {
      blob = response;
    } else {
      // Fallback - assume it's already the blob data
      blob = response;
    }
    ...
    console.log(`AnalyticsAPI: File downloaded successfully: ${filename}`);
    return { success: true, filename };
  } catch (error) {
    console.error('AnalyticsAPI: Failed to download file:', error);
...
export default {
  ...
  exportAnalyticsExcel: withErrorHandling(exportAnalyticsExcel),
  exportAnalyticsPDF: withErrorHandling(exportAnalyticsPDF),
