          ...
          {sortedCampaigns.map((campaign) => (
            <div key={campaign.id} className="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <div className="h-48 bg-gray-100 rounded-t-lg flex items-center justify-center">
                <img src={campaign.files[0]?.url} alt={campaign.name} />
              </div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="font-medium">
                      {calculateProgress(campaign.raised_amount, campaign.target).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                      style={{ width: `${calculateProgress(campaign.raised_amount, campaign.target)}%` }}
                    />
                  </div>
                  <div className="flex justify-between text-sm mt-2 text-gray-600">
                    <span>${orgDashboardApi.formatNumber(campaign.raised_amount || 0)} raised</span>
                    <span>Goal: ${orgDashboardApi.formatNumber(campaign.target || 0)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                  <div className="flex items-center space-x-4">
                    <span className="flex items-center">
                      ...
                      {orgDashboardApi.formatNumber(campaign.views || 0)}
                    </span>
                    <span className="flex items-center">
                      <Users className="w-4 h-4 mr-1" />
                      {orgDashboardApi.formatNumber(campaign.donors_count || 0)}
                    </span>
                  </div>
                  <span className="flex items-center">
